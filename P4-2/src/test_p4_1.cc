...
char *fileName = "Statistics_tmp.txt";
...
void q0 (){
	...
	char *cnf = "SELECT s_suppkey,ps_suppkey FROM a AS b WHERE (s_suppkey = ps_suppkey)";

	yy_scan_string(cnf);
	yyparse();
	final = boolean;
	//PrintComparisonOp(final->left->left);
	//PrintOperand(final->left->left->left);
	//PrintOperand(final->left->left->right);
	//return;
	...
	Statistics s1;
	s1.Read(fileName);	
	cnf = "SELECT s_suppkey FROM a AS b WHERE (s_suppkey>1000)";	
	yy_scan_string(cnf);
	yyparse();
	final = boolean;
...
void q1 (){
	...
	char *cnf = "SELECT s_suppkey FROM a AS b WHERE (l_returnflag = 'R') AND (l_discount < 0.04 OR l_shipmode = 'MAIL')";
...
void q2 (){
	...
	char *cnf = "SELECT s_suppkey FROM a AS b WHERE (c_custkey = o_custkey)";
	yy_scan_string(cnf);
	yyparse();
	final = boolean;
	...
	cnf = " SELECT s_suppkey FROM a AS b WHERE (c_nationkey = n_nationkey)";
	yy_scan_string(cnf);
	yyparse();
	final = boolean;
...
void q3 (){
	...
	char *set1[] ={"s","n1"};
	char *cnf = "SELECT s_suppkey FROM a AS b WHERE (s_nationkey = n_nationkey)";
	yy_scan_string(cnf);
	yyparse();	
	final = boolean;
	...
	char *set3[] = {"c","s","n1","n2"};
	cnf = " SELECT s_suppkey FROM a AS b WHERE (n_nationkey = n_nationkey )";
	yy_scan_string(cnf);
	yyparse();
	final = boolean;
...
void q4 (){
	...
	char *cnf = "SELECT s_suppkey FROM a AS b WHERE (p_partkey=ps_partkey) AND (p_size = 2)";
	yy_scan_string(cnf);
	yyparse();
	final = boolean;
	s.Apply(final, relName, 2);

	cnf ="SELECT s_suppkey FROM a AS b WHERE (s_suppkey = ps_suppkey)";
	yy_scan_string(cnf);
	yyparse();
	final = boolean;
	s.Apply(final, relName, 3);

	cnf =" SELECT s_suppkey FROM a AS b WHERE (s_nationkey = n_nationkey)";
	yy_scan_string(cnf);
	yyparse();
	final = boolean;
	s.Apply(final, relName, 4);

	cnf ="SELECT s_suppkey FROM a AS b WHERE (n_regionkey = r_regionkey) AND (r_name = 'AMERICA') ";
	yy_scan_string(cnf);
	yyparse();
	final = boolean;
...
void q5 (){
	...
	char *cnf = "SELECT s_suppkey FROM a AS b WHERE (c_mktsegment = 'BUILDING')  AND (c_custkey = o_custkey)  AND (o_orderdate < '1995-03-1')";
	yy_scan_string(cnf);
	yyparse();
	final = boolean;
	s.Apply(final, relName, 2);
	...
	cnf = " SELECT s_suppkey FROM a AS b WHERE (l_orderkey = o_orderkey) ";
	yy_scan_string(cnf);
	yyparse();
	final = boolean;
