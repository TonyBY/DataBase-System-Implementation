...
void Function :: Print () {
	...
	for (int i = 0; i < numOps; i++) {
		tmp = opList[i];
		if (!opList[i].name.empty()) {
			std::cout << ArithOpStr[opList[i].myOp] << opList[i].name << " ";
		}
		else {
			if (opList[i].myOp == PushInt) {
				std::cout << ArithOpStr[opList[i].myOp] << *((int*)(opList[i].litInput)) << " ";
			}
			else if (opList[i].myOp == PushDouble){
				std::cout << ArithOpStr[opList[i].myOp] << *((double*)(opList[i].litInput)) << " ";
			}
			else {
				std::cout << ArithOpStr[opList[i].myOp] << " ";
			}
		}
	}
	// std::cout << std::endl;
