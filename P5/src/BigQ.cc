...
BigQ::BigQ(Pipe &inPipe, Pipe &outPipe, OrderMaker &order, int runLen) {
    ...
    //std::cout << "In Join: Pipe (Addr: " << &inPipe << ") -> " << m_sortTmpFilePath << std::endl;
    //m_myS = myS;
...
void BigQ::safeHeapPush(long long idx, Record* pushMe) {
    ...
    delete pushMe;
    //m_heap.push(pair<long long, Record*>(idx, pushMe));
    //pushMe = NULL;
}
...
void BigQ::mergeBlocks(vector<Block>& blocks) {
    ...
    while (nextPop >= 0) {
        ...
        //std::pair<long long, Record*> *tt = (std::pair<long long, Record*>*)&(m_heap.top());
        //std::cout << "tt: " << tt->first << " " << tt->second << std::endl;
        //long long fir = m_heap.top().first;

        //try{
        //if (m_heap.top().first < blocks.size())
        delete m_heap.top().second;
        //}
        //catch(const exception &e) {
        //    std::cout << "[Error] In BigQ::mergeBlocks, " << e.what() << std::endl;
        //}
        //delete tt->second;
        m_heap.pop();
...
void BigQ::TPMMS_Phase2(File &inputFile){
    ...
    */
    ...
    if (m_numRuns == 0) {
        std::cout << "[Error] TPMMS Phase 2: numRuns is zero! (InPipe addr: " << &m_inputPipe << ")" << std::endl;
