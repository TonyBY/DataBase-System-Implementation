...
int add_data (FILE *src, int numrecs, int &res) {
	...
	cout << "After saving to file: " << cnt << endl;
	//cout << "again: " << dbfile.m_file->file->GetLength() << endl;
	dbfile.Close();
	cnt = 0;
	dbfile.Open (rel->path ());
	while (dbfile.GetNext(rec)) {
	...
	cout << "Again After saving to file: " << cnt << endl;
	//cout << "again: " << dbfile.m_file->file->GetLength() << endl;
	dbfile.Close();
	*/
	//cout  << "final close: " << dbfile.Close () << endl;
...
void test3 () {
	...
	/*
	OrderMaker query;
	cout << "build query " << OrderMaker::QueryOrderMaker(query, dbfile.m_metaInfo.order, cnf) << endl;
	query.Print();
	MetaInfo meta = {sorted, 100, query};
	string queryPath = "test_queryOrder";
	dbfile.writeMetaInfo(queryPath, meta);
	return;
	*/
	Record temp;
