...
void q2 () {
	...
	//cout << &(P_p.worker) << " " << &(SF_p.worker) << endl;
...
void q4 () {
	...
	J.Run (_s, _ps, _s_ps, cnf_p_ps, lit_p_ps, numAttsLeft, numAttsRight, attsToKeep, numAttsToKeep, startOfRight);
...
void q6 () {
	...
	GroupBy G;
		...
		Function func;
			...
			//Attribute groupKey[1] = {s_nationkey};
			//int numGroupAtts = 1;
			//Schema grp_sch ("group_sch", numGroupAtts, groupKey);
			string numAtts = "1";
            string atts = "3";
            string types = "Int";
			OrderMaker grp_order (numAtts, atts, types);
			//OrderMaker grp_order (&grp_sch);
	G.Use_n_Pages (1);
